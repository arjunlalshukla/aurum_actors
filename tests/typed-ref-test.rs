use aurum::core::{
  forge, serialize, Case, Host, LocalRef, RegistryMsg, Socket,
  SpecificInterface,
};
use aurum::unified;
...
fn serde_test() {
  ...
  let de_u32 = <LoggerMsg as SpecificInterface<MsgTypes>>::deserialize_as(
    <MsgTypes as Case<u32>>::VARIANT,
    ser_u32,
  );
  assert_eq!(de_u32.unwrap(), LoggerMsg::Error(5));

  let ser_string = serialize("oh no!".to_string()).unwrap();
  let de_string = <LoggerMsg as SpecificInterface<MsgTypes>>::deserialize_as(
  ...
  let de_info = <LoggerMsg as SpecificInterface<MsgTypes>>::deserialize_as(
  ...
  let de_get = <DataStoreMsg as SpecificInterface<MsgTypes>>::deserialize_as(
    <MsgTypes as Case<DataStoreCmd>>::VARIANT,
    ser_get,
  );
  assert!(matches!(
    de_get.unwrap(),
  ...
  let de_put = <DataStoreMsg as SpecificInterface<MsgTypes>>::deserialize_as(
    <MsgTypes as Case<DataStoreCmd>>::VARIANT,
    ser_put,
  );
  assert!(
...
fn forge_test() {
  ...
  let _lgr_msg =
    forge::<MsgTypes, LoggerMsg, LoggerMsg>("logger".to_string(), node.clone());
  println!("logger ref: {:#?}", _lgr_msg);

  let _err_msg =
    forge::<MsgTypes, LoggerMsg, u32>("logger".to_string(), node.clone());
  println!("logger ref u32 interface: {:#?}", _err_msg);

  let _warn_msg =
    forge::<MsgTypes, LoggerMsg, String>("logger".to_string(), node.clone());
